...
def get_block_params_from_plan(plan, layers_dict, layer_params, short_plan=False) -> object:
    ...
    for layer in plan:
        ...
        if layer_type:
            def_params = layer_params.get(layer_type, {}).get("main", {})
            if short_plan:
...
class PlanLinkLibrary:
    custom_block_plan = {
        'Conv2DBNDrop': [(1, 1, 3, 0,
                      {'filters': 32, 'activation': 'relu', 'kernel_size': (3, 3),
                       'padding': 'same', 'strides': (2, 2)}, 0, 0),
                     (2, 6, 2, 0, {}, 1, 0),
                     (3, 6, 1, 0, {'rate': 0.2}, 2, 0)],
    }
...
if __name__ == "__main__":
    # params = get_block_params_for_gui(LayersDef.custom_block_plan['ConvBNDr'])
    # params = GUILayersDef.layers_params.get('ConvBNDr', {}).get('main')
    # print(LayersDef.__dict__)
    # print(getattr(LayersDef, f"{'ConvBNDr'}_defaults"))
