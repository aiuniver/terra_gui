  ...
  >
    <div class="t-modal-datasets">
      <t-field :label="'Количество примеров'">
        <TInputNew v-model.number="example_count" placeholder="Все" type="number" />
      </t-field>
      <template v-for="(block, idx) in inputBlocks">
        <t-field :label="block.name" :key="idx">
          <TAutoComplete :list="filters" placeholder="Выберите датасет" @change="change(block.id, $event)" />
