    ...
    <div class="predictions__params">
      <div class="predictions__param">
        <t-field inline label="Данные для расчета">
          <TSelect :list="sortData" v-model="example_choice_type" small @change="show" />
        </t-field>
        <t-field inline label="Тип выбора данных">
          <TSelect :list="sortOutput" v-model="main_output" small @change="show" />
        </t-field>
        <t-field inline label="Тип выборки">
          <TSelect v-model="data_type" :list="list" small @change="show" />
        </t-field>
      ...
      <div v-if="isYolo" class="predictions__param">
        <t-field inline label="Бокс-канал">
          <TSelect :list="numOutput" v-model="box_channel" small @change="show" />
        </t-field>
        <t-field inline label="Чувствительность">
          <TInputNew v-model="sensitivity" type="number" small style="width: 109px" @change="show" />
        </t-field>
        <t-field inline label="Порог отображения">
          <TInputNew v-model="threashold" type="number" small style="width: 109px" @change="show" />
        </t-field>
      </div>
      <div class="predictions__param">
        <t-field inline label="Выводить промежуточные результаты" :key="'show_results' + show_results" style="margin-top: 5px;">
          <TCheckbox v-model="show_results" small @change="show" />
        </t-field>
        <t-field inline label="Показать статистику" :key="'show_statistic' + show_statistic" style="margin-top: 20px;">
          <TCheckbox v-model="show_statistic" small @change="show" />
        </t-field>
        <!-- <t-field inline label="Фиксация колонок">
          <TCheckbox v-model="fixation" small />
        </t-field> -->
      </div>
