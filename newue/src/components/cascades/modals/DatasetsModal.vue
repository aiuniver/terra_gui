  ...
  >
    <div class="t-modal-datasets">
      <t-field :label="'Количество примеров'">
        <DInputNumber v-model.number="example_count" placeholder="Все" type="number" />
      </t-field>
      <template v-for="(block, idx) in inputBlocks">
        <t-field :label="block.name" :key="idx">
          <Autocomplete2 :list="filters" placeholder="Выберите датасет" @change="change(block.id, $event)" />
