      ...
      <div class="t-statistic__checks">
        <template v-for="({ id, value }, i) of outputLayers">
          <t-field :key="'check_' + i" inline :label="`Выходной слой «${id}»`">
            <DCheckbox small :value="value" @change="change({ id, value: $event.value })" />
          </t-field>
        </template>
      </div>
      <div v-if="isYolo">
        ...
        <t-field inline label="Бокс-канал">
          <DSelect :list="numOutput" v-model="settings.box_channel" small @change="send" />
        </t-field>
      </div>
      <div>
        <t-field inline :label="`Автообновление`">
          <DCheckbox v-model="settings.autoupdate" small @change="send" />
        </t-field>
      </div>
    </div>
