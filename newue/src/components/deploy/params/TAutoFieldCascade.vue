<template>
  <div class="forms">
    ...
    <t-field v-if="type === 'auto_complete'" :label="label">
      <TAutoCompleteTwo :value="getValue" :list="list" :parse="parse" :name="name" all @parse="change" />
    </t-field>
    <t-auto-field-cascade
      v-for="(data, i) of dataFields"
      v-bind="data"
      :key="data.name + i"
      :id="id"
      :type="type"
...
import TAutoCompleteTwo from '../../global/forms/TAutoCompleteTwo.vue';
export default {
  ...
  methods: {
    change({ value, name }) {
      console.log('deploy change', value, name)
