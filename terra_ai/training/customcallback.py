...
class DataProcessing:
    ...
    def image_postprocessing(self):
        pass

    def text_postrocessing(self):
        pass
...
class TrainingProcessData:
    ...
    def set_state(self, model, x, y, num_examples_plot, ex_type_choise, num_classes):
        self.model = model
        self.x = x
        self.y_true = y
        self.num_classes = num_classes
        self.num_examples = num_examples_plot
        self.ex_type_choise = ex_type_choise
        self.y_pred = self.model.predict(self.x)

    def get_metrics_result(self):
        for metric_name, metric_obj in self.metrics.items():
            if metric_name == 'MeanIoU':
                m = metric_obj(self.num_classes)
            else:
                m = metric_obj()
            m.update_state(self.y_true, self.y_pred)
            self.metrics_data[metric_name] = m.result().numpy()
...
class InteractiveCallback:
    ...
    def __init__(self, null_model, dataset, loss_name="CategoricalCrossentropy", custom_dataset=None):
        """
