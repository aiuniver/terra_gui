...
class DTS(object):
    ...
    def get_parameters_dict(self):
        ...
        parameters = {}
        list_of_params = ["images", "text", "audio", "dataframe"] + [
            "classification",
            "regression",
            "segmentation",
            "text_segmentation",
        ...
        for elem in list_of_params:
            ...
            for key, value in self.call_method(elem).items():
                if type(value) == list:
                    ...
                    temp[key] = {"type": type(value).__name__, "default": value}
    ...
    def load_dataset(self):
        ...
        zip_name = widgets.Dropdown(
            options=filelist, value=filelist[0], description="Файл", disabled=False
        )
        google_drive = widgets.VBox([zip_name, button])

        # Вторая вкладка
        url_google = widgets.Text(
            value="", placeholder="https://", description="URL:", disabled=False
        )
        ...
        terra_dataset = widgets.RadioButtons(
            options=datasets,
            value=datasets[0],
    ...
    def create_dataset(self, inputs, outputs):
        def build_widget(x):
            ...
            def get_text(key, value):
                text_widget = widgets.Text(
                    value=value, placeholder="", description=key, disabled=False
                )
                return text_widget

            def get_int(key, value):
                integer_widget = widgets.IntText(
                    value=value, description=key, disabled=False
                )
            ...
            def get_dropdown(key, value):
                dropdown_widget = widgets.Dropdown(
                    options=value, value=value[0], description=key, disabled=False
                )
            ...
            def manual_input(x):
                ...
                blocks = []
                for i in range(x):
                    color_name = widgets.Text(
                        value="",
                        placeholder="",
                        description=f"Класс {i + 1}",
                        disabled=False,
                    )
                    color_choose = widgets.ColorPicker(
                        concise=False,
                        description="Цвет",
                        value="#FFFFFF",
                        disabled=False,
            ...
            def get_color(x):
                def find_classes(b):
                    ...
                    for i in range(len(list_of_outputs)):
                        if list_of_outputs[i].children[3].kwargs["x"] == "segmentation":
                            folder_name = (
                                list_of_outputs[i].children[3].result.children[0].value
                            )
                            mask_range = (
                                list_of_outputs[i].children[3].result.children[1].value
                            )

                    color_list = self._find_colors(
                        folder_name, auto_classes.value, mask_range
                    )
                    color_widgets = []
                ...
                if x == "Ручной ввод":
                    data = widgets.interactive(
                        manual_input,
                        x=widgets.IntText(
                            value=1, description="Кол-во классов", disabled=False
                        ),
                    )
                    display(data)
                elif x == "Автоматический поиск":
                    global auto_classes
                    auto_classes = widgets.IntText(
                        value=1, description="Кол-во классов", disabled=False
                    )
                    auto_button = widgets.Button(
                        description="Поиск", disabled=False, button_style="", icon=""
                    )
                    ...
                    data = widgets.Dropdown(
                        options=os.listdir(self.file_folder),
                        value=os.listdir(self.file_folder)[0],
            ...
            for i, param in enumerate(input.keys(), 1):
                ...
                if input[param] is None or isinstance(input[param], str):
                    ...
                    isinstance(input[param], dict)
                    and data_type_out.value == "segmentation"
                ):
        ...
        def create_dataset(b):
            ...
            for i in range(len(list_of_inputs)):
                ...
                for j in range(len(list_of_inputs[i].children[2].result.children)):
                    params[
                        list_of_inputs[i].children[2].result.children[j].description
            ...
            for i in range(len(list_of_outputs)):
                ...
                if dic["tag"] == "segmentation":
                    ...
                    classes_dict = {}
                    if (
                        list_of_outputs[i].children[3].result.children[2].kwargs["x"]
                        == "Ручной ввод"
                    ):
                        for j in range(
                            list_of_outputs[0]
                        ...
                        classes_dict = self._find_colors(
                            list_of_outputs[i]
                            .children[3]
                            .result.children[2]
                            .result.value,
        ...
        for i in range(1, inputs + 1):
            title = widgets.HTML(
                value=f"<b>{f'input_{i}':-^67}</b>", placeholder="", description=""
            )
            name = widgets.Text(
                value="",
                placeholder="Название входа",
                description=f"input_{i}",
                disabled=False,
            )
        ...
        for i in range(1, outputs + 1):
            title = widgets.HTML(
                value=f"<b>{f'output_{i}':-^67}</b>", placeholder="", description=""
            )
            name = widgets.Text(
                value="",
                placeholder="Название выхода",
                description=f"output_{i}",
                disabled=False,
            )
            task_type = widgets.Dropdown(
                options=[
                    ("Классификация", "classification"),
                    ("Регргессия", "regression"),
            ...
            data_type_out = widgets.Dropdown(
                options=[
                    ("Картинки", "images"),
                    ("Текст", "text"),
                    ("Аудио", "audio"),
        ...
        slider = widgets.IntSlider(
            description="Train:", value=80, step=1, min=5, max=95
        )
        slider2 = widgets.IntSlider(
            description="Val+test:", value=20, step=1, min=5, max=95
        )
        ...
        button = widgets.Button(
            description="Сформировать", disabled=False, button_style="", icon="check"
        )
    ...
    def create_custom_dataset(self, inputs, outputs, globals):
        def send_arrays(b):
            ...
            full_val = len(
                set(
                    [bool(globals[f"xval_{i}"].value) for i in range(inputs)]
                    + [bool(globals[f"yval_{i}"].value) for i in range(outputs)]
                )
            )
            full_test = len(
                set(
                    [bool(globals[f"xtest_{i}"].value) for i in range(inputs)]
                    + [bool(globals[f"ytest_{i}"].value) for i in range(outputs)]
                )
            )
            if full_val != 1:
                assert (
                    full_val == 1
                ), "Колонка валидационной выборки заполнена не полностью."
            if full_test != 1:
                assert (
                    full_test == 1
                ), "Колонка тестовой выборки заполнена не полностью."
            ...
            x_scaler = {}
            for i in range(inputs):
                x_scaler[globals[f"x_name_{i}"].description[:-1]] = globals[
                    f"scaler_x_{i}"
                ].value

            y_scaler = {}
            for i in range(outputs):
                y_scaler[globals[f"y_name_{i}"].description[:-1]] = globals[
                    f"scaler_y_{i}"
                ].value
            ...
            x_shape = {}
            for i in range(inputs):
                x_shape[globals[f"x_name_{i}"].description[:-1]] = globals[
                    f"net_type_x_{i}"
                ].value

            y_shape = {}
            for i in range(outputs):
                y_shape[globals[f"y_name_{i}"].description[:-1]] = globals[
                    f"net_type_y_{i}"
                ].value
                if ohe[globals[f"y_name_{i}"].description[:-1]]:
            ...
            if checkbox_google.value:
                ...
                with open(f"{os.path.join(directory, self.name)}.trds", "wb") as f:
                    dill.dump(self, f)
                print(
                    f"Датасет сохранен в файл {os.path.join(directory, self.name)}.trds"
                )
        ...
        for i in range(inputs):
            list_of_widgets = []
            globals[f"input_{i}"] = {}
            globals[f"x_name_{i}"] = widgets.Text(
                value="",
                placeholder="Название входа",
                description=f"input_{i + 1}:",
                disabled=False,
            )
            ...
            globals[f"x_tag_{i}"] = widgets.Dropdown(
                options=[
                    ...
                    ("Датафрейм", "regression"),
                    ("Другое", "other"),
                ],
                value="other",
                description=f"Тип данных:",
                disabled=False,
            )
            list_of_widgets.append(globals[f"x_tag_{i}"])
            globals[f"xtrain_{i}"] = widgets.Text(
                value="x_train",
                description="X/Train:",
                placeholder="X или x_train",
                disabled=False,
            )
            list_of_widgets.append(globals[f"xtrain_{i}"])
            globals[f"xval_{i}"] = widgets.Text(
                value="x_val", description="Validation:", placeholder="", disabled=False
            )
            list_of_widgets.append(globals[f"xval_{i}"])
            disabled_list.append(globals[f"xval_{i}"])
            globals[f"xtest_{i}"] = widgets.Text(
                value="x_test", description="Test:", placeholder="", disabled=False
            )
        ...
        for i in range(outputs):
            list_of_widgets = []
            globals[f"output_{i}"] = {}
            globals[f"y_name_{i}"] = widgets.Text(
                value="",
                placeholder="Название выхода",
                description=f"output_{i + 1}:",
                disabled=False,
            )
            ...
            globals[f"y_task_{i}"] = widgets.Dropdown(
                options=[
                    ...
                    ("Временные ряды", "timeseries"),
                    ("Предсказание временного ряда", "timeseries_prediction"),
                ],
                value="classification",
                description="Тип задачи:",
            )
            globals[f"y_tag_task_{i}"] = widgets.VBox(
                [globals[f"y_tag_{i}"], globals[f"y_task_{i}"]]
            )
            list_of_widgets.append(globals[f"y_tag_task_{i}"])
            globals[f"ytrain_{i}"] = widgets.Text(
                value="y_train",
                description="Y/Train:",
                placeholder="Y или y_train",
                disabled=False,
            )
            list_of_widgets.append(globals[f"ytrain_{i}"])
            globals[f"yval_{i}"] = widgets.Text(
                value="y_val", description="Validation:", placeholder="", disabled=False
            )
            list_of_widgets.append(globals[f"yval_{i}"])
            disabled_list.append(globals[f"yval_{i}"])
            globals[f"ytest_{i}"] = widgets.Text(
                value="y_test", description="Test:", placeholder="", disabled=False
            )
        ...
        checkbox_google = widgets.Checkbox(
            value=True, description="Сохранить в Google Drive", disabled=False
        )
        checkbox_split = widgets.Checkbox(
            value=False, description="Train/Val/Test split", disabled=False
        )
        ...
        for i in range(inputs):
            list_of_widgets_2 = []
            globals[f"scaler_x_{i}"] = widgets.Dropdown(
                options=["Не применять", "StandardScaler", "MinMaxScaler"],
                value="Не применять",
                description=f"x_Scaler_{i + 1}:",
                disabled=False,
            )
            list_of_widgets_2.append(globals[f"scaler_x_{i}"])
            globals[f"net_type_x_{i}"] = widgets.Dropdown(
                options=["Без изменений", "Добавить размерность", "Выпрямить"],
                value="Без изменений",
        ...
        for i in range(outputs):
            list_of_widgets_2 = []
            globals[f"scaler_y_{i}"] = widgets.Dropdown(
                options=["Не применять", "StandardScaler", "MinMaxScaler"],
                value="Не применять",
                description=f"y_Scaler_{i + 1}:",
                disabled=False,
            )
            ...
            globals[f"ohe_{i}"] = widgets.Checkbox(
                value=False, description="One-Hot Encoding", disabled=False
            )
            globals[f"block_{i}"] = widgets.HBox(
                [*list_of_widgets_2, globals[f"ohe_{i}"]]
            )
            list_of_rows_2.append(globals[f"block_{i}"])
    ...
    def _set_datatype(self, **kwargs):
        ...
        dtype = {1: "DIM", 2: "DIM", 3: "1D", 4: "2D", 5: "3D"}
    ...
    def _find_colors(self, name, num_classes=None, mask_range=None, txt_file=False):
        ...
        if txt_file:
            ...
            for i in range(len(txt)):
                color_list[txt.loc[i, "# label"]] = [
                    int(num) for num in txt.loc[i, "color_rgb"].split(",")
                ]
        else:
            color_list = []
                        ...
                        if color not in color_list:
                            for in_color in color_list:
                                if (
                                    color[0]
                                    in range(
                                        in_color[0] - mask_range,
                                        in_color[0] + mask_range,
                                    )
                                    and color[1]
                                    in range(
                                        in_color[1] - mask_range,
                                        in_color[1] + mask_range,
                                    )
                                    and color[2]
                                    in range(
                                        in_color[2] - mask_range,
                                        in_color[2] + mask_range,
                                    )
                                ):
                                    add_condition = False
    ...
    def load_data(self, name, mode, link=None) -> None:
        ...
        if mode == "google_drive":
            filepath = os.path.join(
                "/content/drive/MyDrive/TerraAI/datasets/sources", name
            )
            ...
            if name in data.keys():
                ...
                for base in data[name]:
                    ...
                    if not file_folder.joinpath("tmp").exists():
                        os.makedirs(os.path.join(file_folder, "tmp"))
                    link = (
                        "https://storage.googleapis.com/terra_ai/DataSets/Numpy/" + base
                    )
                    with request.urlopen(link) as dl_file:
                        with open(
                            os.path.join(default_path, name, "tmp", base), "wb"
                        ) as out_file:
                            out_file.write(dl_file.read())
                    if "zip" in base:
                ...
                if not name in data.keys():
                    if self.django_flag:
                        self.Exch.print_error(
                            ("Error", "Данной базы нет в списке готовых баз.")
                        )
                    else:
                        assert (
                            name in data.keys()
                        ), "Данной базы нет в списке готовых баз."
        self.name = name
    ...
    def prepare_dataset(self, **options):
        def load_arrays():
            ...
            for arr in os.listdir(os.path.join(self.file_folder, "arrays")):
                if arr[0] == "X":
                    self.X[arr[2:-3]] = {
                        "data_name": f"Вход_{arr[-4]}",
                        "data": joblib.load(
                            os.path.join(self.file_folder, "arrays", arr)
                        ),
                    }
                    self.input_shape[arr[2:-3]] = self.X[arr[2:-3]]["data"][0].shape[1:]
                    inp_datatype.append(
                        self._set_datatype(shape=self.X[arr[2:-3]]["data"][0].shape)
                    )
                    self.tags[arr[2:-3]] = tag_list[0]
                elif arr[0] == "Y":
                    self.Y[arr[2:-3]] = {
                        "data_name": f"Выход_{arr[-4]}",
                        "data": joblib.load(
                            os.path.join(self.file_folder, "arrays", arr)
                        ),
                    }
        ...
        def load_scalers():
            ...
            if "scalers" in os.listdir(self.file_folder):
                X_scalers = [
                    sclr[2:-3]
                    for sclr in os.listdir(os.path.join(self.file_folder, "scalers"))
                    if "X" in sclr
                ]
                Y_scalers = [
                    sclr[2:-3]
            ...
            for inp in list_of_inputs:
                if inp in X_scalers:
                    self.x_Scaler[inp] = joblib.load(
                        os.path.join(self.file_folder, "scalers", f"X_{inp}.gz")
                    )
                else:
            ...
            for out in list_of_outputs:
                if out in Y_scalers:
                    self.y_Scaler[out] = joblib.load(
                        os.path.join(self.file_folder, "scalers", f"Y_{out}.gz")
                    )
                else:
        ...
        def load_tokenizer():
            ...
            if "tokenizer" in os.listdir(self.file_folder):
                tokenizer = [
                    tok[0:-3]
                    for tok in os.listdir(os.path.join(self.file_folder, "tokenizer"))
                ]
            else:
                tokenizer = []
            ...
            for inp in list_of_inputs:
                if inp in tokenizer:
                    self.tokenizer[inp] = joblib.load(
                        os.path.join(self.file_folder, "tokenizer", f"{inp}.gz")
                    )
                else:
        ...
        def load_word2vec():
            ...
            if "word2vec" in os.listdir(self.file_folder):
                word2v = [
                    w2v[0:-3]
                    for w2v in os.listdir(os.path.join(self.file_folder, "word2vec"))
                ]
            else:
                word2v = []
            ...
            for inp in list_of_inputs:
                if inp in word2v:
                    self.word2vec[inp] = joblib.load(
                        os.path.join(self.file_folder, "word2vec", f"{inp}.gz")
                    )
                else:
        ...
        if options["dataset_name"] in [
            ...
            if options["dataset_name"] in [
                ...
                self.keras_datasets(
                    options["dataset_name"], one_hot_encoding=True, test=True
                )
                self.task_type["output_1"] = "classification"
                ...
                self.keras_datasets(
                    options["dataset_name"], scaler="StandardScaler", test=True
                )
            ...
            self.source_datatype = literal_eval(
                config.get("ATTRIBUTES", "source_datatype")
            )
            self.source_shape = literal_eval(config.get("ATTRIBUTES", "source_shape"))
            self.classes_names["output_1"] = literal_eval(
                config.get("ATTRIBUTES", "classes_names")
            )
            self.classes_colors["output_1"] = literal_eval(
                config.get("ATTRIBUTES", "classes_colors")
            )
            ...
            self.one_hot_encoding = literal_eval(
                config.get("ATTRIBUTES", "one_hot_encoding")
            )
            ...
            for i, folder in enumerate(progress_bar):
                ...
                if self.django_flag:
                    ...
                    if idx == progress_bar.total:
                        self.Exch.print_progress_bar(
                            progress_bar_status, stop_flag=True
                        )
                    else:
    ...
    def prepare_custom_dataset(self, *Data, **options):
        ...
        if options["split"]:
            ...
            for inp in inputs:
                self.X[inp]["data"] = (
                    Data[0][inp]["data"][0][train_mask],
                    Data[0][inp]["data"][0][val_mask],
                    Data[0][inp]["data"][0][test_mask],
                )
            for out in outputs:
                self.Y[out]["data"] = (
                    Data[1][out]["data"][0][train_mask],
                    Data[1][out]["data"][0][val_mask],
        ...
        for inp in inputs:
            ...
            self.source_datatype[inp] = self._set_datatype(
                shape=self.X[inp]["data"][0].shape
            )
                    ...
                    for array in self.X[inp]["data"]:
                        if isinstance(array, np.ndarray):
                            list_of_arrays.append(
                                array.reshape(-1, np.prod(array.shape[1:]))
                            )
                        else:
                            list_of_arrays.append(None)
                    self.X[inp]["data"] = tuple(list_of_arrays)
    ...
    def keras_datasets(self, dataset, **options):
            ...
            if name in pics:
                ...
                for i in range(10):
                    ...
                    if name in ["mnist", "fashion_mnist"]:
                        ...
                        axs[i].axis("off")
                        axs[i].set_title(
                            f'{i}: {self.classes_names["input_1"][title[0]]}'
                        )
        ...
        self.tags = {"input_1": tags[0], "output_1": tags[1]}
        ...
        if not dataset.lower() in data.keys():
            self.Exch.print_error(
                ("Error", "Данного датасета нет в списке стандартных датасетов keras.")
            )
            if not self.django_flag:
                assert (
                    dataset.lower() in data.keys()
                ), "Данного датасета нет в списке стандартных датасетов keras."
        ...
        self.source_shape["input_1"] = (
            x_Train.shape if len(x_Train.shape) < 2 else x_Train.shape[1:]
        )
        ...
        if "classification" in self.tags["output_1"]:
            ...
            if self.name == "fashion_mnist":
                ...
                self.classes_names["input_1"] = [
                    str(i) for i in range(len(np.unique(y_Train, axis=0)))
                ]
        ...
        self.input_shape["input_1"] = (
            x_Train.shape if len(x_Train.shape) < 2 else x_Train.shape[1:]
        )
        self.input_datatype = self._set_datatype(shape=x_Train.shape)

        if (
            "scaler" in options.keys()
            and options["scaler"] == "MinMaxScaler"
            or "scaler" in options.keys()
            and options["scaler"] == "StandardScaler"
        ):
        ...
        if "one_hot_encoding" in options.keys() and options["one_hot_encoding"] == True:
            if "classification" in self.tags["output_1"]:
                ...
                if not self.django_flag:
                    print(
                        f"One-Hot encoding only available for classification which {self.name} was not meant for. "
                        f"One-Hot encoding was not implemented."
                    )

        self.X = {"input_1": {"data_name": "Вход", "data": (x_Train, x_Val, None)}}
        self.Y = {"output_1": {"data_name": "Выход", "data": (y_Train, y_Val, None)}}
        ...
        if "test" in options.keys() and options["test"] is True:
            ...
            x_Val, x_Test, y_Val, y_Test = train_test_split(
                x_Val, y_Val, test_size=1 - split_size, shuffle=True
            )
        ...
        if not self.django_flag:
            print(
                f"Формирование массивов завершено. Времени затрачено: {round(time() - cur_time, 2)} сек."
            )
        ...
        for _, dirnames, filename in sorted(os.walk(folder_name)):
            ...
            for i in range(folders_num):
                ...
                for j in range(len(self.user_parameters["out"])):
                    if (
                        self.user_parameters["out"][f"output_{j+1}"]["tag"]
                        == "object_detection"
                    ):

                        data = {}
                        with open(
                            os.path.join(self.file_folder, "obj.data"), "r"
                        ) as dt:
                            d = dt.read()
                        for elem in d.split("\n"):
                            if elem:
                        ...
                        with open(
                            os.path.join(
                                self.file_folder, data["train"].split("/")[-1]
                            ),
                            "r",
                        ) as dt:
                            imgs = dt.read()
                ...
                if len(self.source_shape) < 3:
                    source_shape = Image.open(
                        os.path.join(temp_path, os.listdir(temp_path)[0])
                    ).size
                    self.source_shape[f"input_{self.iter}"] = (
                        source_shape[1],
                        source_shape[0],
                        3,
                    )
    ...
    def text(
        ...
        def read_text(file_path):
            ...
            for i in range(len(self.user_parameters["out"])):
                if (
                    ...
                    open_symb = self.user_parameters["out"][f"output_{i+1}"][
                        "parameters"
                    ]["open_tags"].split(" ")[0][0]
                    close_symb = self.user_parameters["out"][f"output_{i+1}"][
                        "parameters"
                    ]["open_tags"].split(" ")[0][-1]
        ...
        for i in range(len(self.user_parameters["out"])):
            if (
                ...
                open_tags = self.user_parameters["out"][f"output_{i + 1}"][
                    "parameters"
                ]["open_tags"]
                close_tags = self.user_parameters["out"][f"output_{i + 1}"][
                    "parameters"
                ]["close_tags"]
        ...
        if embedding:
            ...
            self.word2vec[f"input_{self.iter}"] = word2vec.Word2Vec(
                x, size=embedding_size, window=10, min_count=1, workers=10, iter=10
            )
    ...
    def dataframe(
        ...
        if separator:
            ...
            X = pd.read_csv(
                os.path.join(self.file_folder, file_name), encoding=encoding
            )
        self.df = X
    ...
    def regression(
        self, y_col="", scaler=["No Scaler", "StandardScaler", "MinMaxScaler"]
    ):
    ...
    def timeseries(
        ...
        time_Train = TimeseriesGenerator(
            x_Train, x_Train, length=x_len, stride=1, batch_size=batch_size
        )
        time_Val = TimeseriesGenerator(
            x_Val, x_Val, length=x_len, stride=1, batch_size=batch_size
        )
    ...
    def audio(
        ...
        self.classes_names[f"input_{self.iter}"] = (
            [folder for folder in sorted(os.listdir(folder_name))]
        ...
        for _, dirnames, filename in sorted(os.walk(folder_name)):
            ...
            for i in range(folders_num):
                ...
                files = [
                    os.path.join(temp_path, wavfile)
                    for wavfile in sorted(os.listdir(temp_path))
                ]
    ...
    def text_segmentation(self, open_tags="", close_tags=""):
        ...
        for i in range(len(self.user_parameters["inp"])):
            if self.user_parameters["inp"][f"input_{i+1}"]["tag"] == "text":
                x_len = self.user_parameters["inp"][f"input_{i+1}"]["parameters"][
                    "x_len"
                ]
                step = self.user_parameters["inp"][f"input_{i+1}"]["parameters"]["step"]
                tags_indexes = np.array(
                    [self.tokenizer[f"input_{i+1}"].word_index[k] for k in tags]
                )
                break
        ...
        for i in range(len(self.user_parameters["inp"])):
            if self.user_parameters["inp"][f"input_{i + 1}"]["tag"] == "text":
                if self.user_parameters["inp"][f"input_{i + 1}"]["parameters"][
                    "embedding"
                ]:
                    reversed_tok = {}
    ...
    def segmentation(
        ...
        folder_name=[""],
        mask_range=10,
        classes_dict={"название класса": [0, 0, 0]},
    ):
        def load_image(img_path, shape):
        ...
        def cluster_to_ohe(image):
            ...
            for k, (name, rgb) in enumerate(classes_dict.items()):
                ...
                for j, cl_rgb in enumerate(cl_cent):
                    if (
                        rgb[0] in range(cl_rgb[0] - mask_range, cl_rgb[0] + mask_range)
                        and rgb[1]
                        in range(cl_rgb[1] - mask_range, cl_rgb[1] + mask_range)
                        and rgb[2]
                        in range(cl_rgb[2] - mask_range, cl_rgb[2] + mask_range)
                    ):
                        mask = cl_mask[:, :, j]
        ...
        for i in range(len(self.user_parameters["inp"])):
            if self.user_parameters["inp"][f"input_{i+1}"]["tag"] == "images":
                height = self.user_parameters["inp"][f"input_{i+1}"]["parameters"][
                    "height"
                ]
                width = self.user_parameters["inp"][f"input_{i+1}"]["parameters"][
                    "width"
                ]
                shape = (height, width)
                break
    ...
    def object_detection(self):
        def make_y(real_boxes, num_classes):
            ...
            y_true = [
                np.zeros(
                    (cells[l], cells[l], len(anchor_mask[l]), 5 + num_classes),
                    dtype="float32",
                )
            ...
            for r in range(len(real_boxes)):
                correct_anchors = []
                for elem in anchors:
                    correct_anchors.append(
                        [min(elem[0], boxes_wh[r][0]), min(elem[1], boxes_wh[r][1])]
                    )
                correct_anchors = np.array(correct_anchors)
                correct_anchors_area = correct_anchors[:, 0] * correct_anchors[:, 1]
                iou = correct_anchors_area / (
                    box_area[r] + anchor_area - correct_anchors_area
                )
        ...
        # obj.names
        with open(
            os.path.join(self.file_folder, data["names"].split("/")[-1]), "r"
        ) as dt:
            names = dt.read()
        ...
        txt_list = []
        with open(
            os.path.join(self.file_folder, data["train"].split("/")[-1]), "r"
        ) as dt:
            imgs = dt.read()
        for elem in imgs.split("\n"):
            if elem:
    ...
    def prepare_user_dataset(self, dataset_dict, is_save=True):
                ...
                for param_key, param_value in value["parameters"].items():
                    try:
                        ...
                        if param_value == "true" or param_value == "on":
                            ...
                            dataset_dict["inputs"][key]["parameters"][param_key] = int(
                                param_value
                            )
                    except ValueError:
                        continue
                ...
                for param_key, param_value in value["parameters"].items():
                    try:
                        ...
                        if param_value == "true" or param_value == "on":
                            ...
                            dataset_dict["outputs"][key]["parameters"][
                                param_key
                            ] = False
                        else:
                            dataset_dict["outputs"][key]["parameters"][param_key] = int(
                                param_value
                            )
                    except ValueError:
                        continue
        ...
        self.divide_ratio[1] = (
            dataset_dict["parameters"]["train_part"],
            dataset_dict["parameters"]["val_part"],
        ...
        for i in range(len(self.user_parameters["inp"])):
            self.tags[f"input_{i + 1}"] = dataset_dict["inputs"][f"input_{i + 1}"][
                "tag"
            ]
        for i in range(len(self.user_parameters["out"])):
            self.tags[f"output_{i + 1}"] = dataset_dict["outputs"][f"output_{i + 1}"][
                "tag"
            ]
            self.task_type[f"output_{i + 1}"] = dataset_dict["outputs"][
                f"output_{i + 1}"
            ]["task_type"]
        ...
        for i in range(len(self.user_parameters["inp"])):
            self.iter = i + 1
            self.X[f"input_{i+1}"] = {
                "data_name": self.user_parameters["inp"][f"input_{i+1}"]["name"],
                "data": getattr(
                    self, self.user_parameters["inp"][f"input_{i+1}"]["tag"]
        ...
        for i in range(len(self.user_parameters["out"])):
            ...
            if (
                ...
                outputs = getattr(
                    self, self.user_parameters["out"][f"output_{i+1}"]["tag"]
                )(**self.user_parameters["out"][f"output_{i+1}"]["parameters"])
                for k in range(3):
                    self.Y[f"output_{i+k+1}"] = {
                        "data_name": self.user_parameters["out"][f"output_{i+1}"][
                            "name"
                        ],
                        "data": outputs[k],
                    }
                    self.tags[f"output_{i+k+1}"] = dataset_dict["outputs"][
                        f"output_{i+1}"
                    ]["tag"]
                    self.task_type[f"output_{i+k+1}"] = dataset_dict["outputs"][
                        f"output_{i+1}"
                    ]["task_type"]
            else:
                self.Y[f"output_{i+1}"] = {
                    "data_name": self.user_parameters["out"][f"output_{i+1}"]["name"],
                    "data": getattr(
                        self, self.user_parameters["out"][f"output_{i+1}"]["tag"]
                    )(**self.user_parameters["out"][f"output_{i+1}"]["parameters"]),
                }
        ...
        for inp in self.X.keys():
            self.X[inp]["data"] = (
                self.X[inp]["data"][train_mask],
                self.X[inp]["data"][val_mask],
                self.X[inp]["data"][test_mask],
            )
        for out in self.Y.keys():
            self.Y[out]["data"] = (
                self.Y[out]["data"][train_mask],
                self.Y[out]["data"][val_mask],
            ...
            for inp in self.X.keys():
                for i, item in enumerate(self.X[inp]["data"]):
                    if isinstance(item, np.ndarray):
                        print(
                            f'Размерность {inp} - {x[i]}: {self.X[inp]["data"][i].shape}'
                        )
            for out in self.Y.keys():
                for i, item in enumerate(self.Y[out]["data"]):
                    if isinstance(item, np.ndarray):
                        print(
                            f'Размерность {out} - {y[i]}: {self.Y[out]["data"][i].shape}'
                        )
