...
class DTS(object):
    ...
    def prepare_dataset(self, **options):
        ...
        if options['dataset_name'] in ['mnist', 'fashion_mnist', 'cifar10', 'cifar100', 'imdb', 'boston_housing', 'reuters']:
            ...
            if options['dataset_name'] in ['трейдинг', 'умный_дом', 'квартиры', 'автомобили', 'автомобили_3',
                ...
                self.file_folder = os.path.join(self.trds_path, options['dataset_name'])
    ...
    def prepare_user_dataset(self, dataset_dict: dict, is_save=True):
        ...
        if is_save:
            ...
            os.makedirs(os.path.join(self.trds_path, self.name), exist_ok=True)
            if self.X:
                os.makedirs(os.path.join(self.trds_path, self.name, 'arrays'), exist_ok=True)
            if self.scaler:
                os.makedirs(os.path.join(self.trds_path, self.name, 'scalers'), exist_ok=True)
            if self.tokenizer:
                os.makedirs(os.path.join(self.trds_path, self.name, 'tokenizer'), exist_ok=True)
            if self.word2vec:
                os.makedirs(os.path.join(self.trds_path, self.name, 'word2vec'), exist_ok=True)
            if self.tsgenerator:
                os.makedirs(os.path.join(self.trds_path, self.name, 'tsgenerator'), exist_ok=True)

            for arr in self.X.keys():
                if self.X[arr]:
                    joblib.dump(self.X[arr], os.path.join(self.trds_path, self.name, 'arrays', f'{arr}.gz'))
            for arr in self.Y.keys():
                if self.Y[arr]:
                    joblib.dump(self.Y[arr], os.path.join(self.trds_path, self.name, 'arrays', f'{arr}.gz'))
            for sclr in self.scaler.keys():
                if self.scaler[sclr]:
                    joblib.dump(self.scaler[sclr], os.path.join(self.trds_path, self.name, 'scalers', f'{sclr}.gz'))
            for tok in self.tokenizer.keys():
                if self.tokenizer[tok]:
                    joblib.dump(self.tokenizer[tok], os.path.join(self.trds_path, self.name, 'tokenizer', f'{tok}.gz'))
            for w2v in self.word2vec.keys():
                if self.word2vec[w2v]:
                    joblib.dump(self.word2vec[w2v], os.path.join(self.trds_path, self.name, 'word2vec', f'{w2v}.gz'))
            for tsg in self.tsgenerator.keys():
                if self.tsgenerator[tsg]:
                    joblib.dump(self.tsgenerator[tsg], os.path.join(self.trds_path, self.name, 'tsgenerator', f'{tsg}.gz'))
            ...
            data['size'] = self._get_size(os.path.join(self.trds_path, self.name))
            with open(os.path.join(self.trds_path, self.name, 'config.json'), 'w') as fp:
                json.dump(data, fp)
            print(f'Файлы датасета сохранены в папку {os.path.join(self.trds_path, self.name)}')
            print(f'Json сохранен в файл {os.path.join(self.trds_path, self.name, "config.json")}')
