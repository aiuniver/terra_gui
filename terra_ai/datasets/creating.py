...
class CreateDTS(object):
    ...
    def create_one_file_array(self, file_data: OneFileData, source_path):
        instructions_data = getattr(self, f"instructions_{decamelize(file_data.type)}")(
            file_data
        )
    ...
    def create_dataset(self, creation_data: CreationData):
        ...
        if not creation_data.use_generator:
            ...
            x_array = self.create_dataset_arrays(
                creation_data=creation_data, put_data=self.instructions.inputs
            )
            y_array = self.create_dataset_arrays(
                creation_data=creation_data, put_data=self.instructions.outputs
            )
        ...
        output = DatasetData(
            **self.create_dataset_configure(creation_data=creation_data)
        )
    ...
    def set_paths(self, data: CreationData) -> PathsData:
        ...
        return PathsData(
            datasets=dataset_path, instructions=instructions_path, arrays=arrays_path
        )
    ...
    def create_put_instructions(
        self, data: Union[CreationInputsList, CreationOutputsList]
    ) -> dict:
            ...
            if decamelize(elem.type) not in ["text_segmentation"]:
                for paths in elem.parameters.sources_paths:
                    if not paths.suffix == ".csv":
                        for directory, folder, file_name in sorted(
                            os.walk(os.path.join(self.file_folder, paths))
                        ):
                            if file_name:
                                file_folder = directory.replace(self.file_folder, "")[
                                    1:
                                ]
                                for name in sorted(file_name):
                                    paths_list.append(os.path.join(file_folder, name))
                    elif paths.suffix == ".csv" and elem.type not in [
                        "Dataframe",
                        "Timeseries",
                    ]:
                        data = pd.read_csv(
                            os.path.join(self.file_folder, paths),
                            usecols=elem.parameters.cols_names,
                        )
                        paths_list = data[elem.parameters.cols_names[0]].to_list()
            instructions_data = InstructionsData(
                **getattr(self, f"instructions_{decamelize(elem.type)}")(
                    paths_list, elem
                )
            )
    ...
    def write_instructions_to_files(self):
        ...
        for put in self.instructions.__dict__.keys():
            for idx in self.instructions.__dict__[put].keys():
                with open(
                    os.path.join(
                        self.paths.instructions, "parameters", f"{idx}_{put}.json"
                    ),
                    "w",
                ) as cfg:
        ...
        for key in self.dataframe.keys():
            self.dataframe[key].to_csv(
                os.path.join(self.paths.instructions, "tables", f"{key}.csv")
            )
            ...
            if preprocess:
                ...
                for key in preprocess.keys():
                    if preprocess.get(key, {}):
                        joblib.dump(
                            preprocess[key],
                            os.path.join(preprocess_file_path, f"{key}.gz"),
                        )
        ...
        for key in self.instructions.inputs.keys():
            ...
            if isinstance(array, tuple):
                for i in range(len(array)):
                    current_input = DatasetInputsData(
                ...
                current_input = DatasetInputsData(
                    datatype=DataType.get(len(array.shape), "DIM"),
    ...
    def create_output_parameters(self, creation_data: CreationData) -> dict:
        ...
        for key in self.instructions.outputs.keys():
            ...
            if isinstance(array, tuple):
                for i in range(len(array)):
                    current_output = DatasetOutputsData(
                        datatype=DataType.get(len(array[i].shape), "DIM"),
                        dtype=str(array[i].dtype),
                        shape=array[i].shape,
    ...
    def create_table(self, creation_data: CreationData):
        ...
        for key, value in split_sequence.items():
            self.dataframe[key] = array_creator.df_ts.loc[value, :].reset_index(
                drop=True
            )

    def create_dataset_arrays(
        self, creation_data: CreationData, put_data: dict
    ) -> dict:
        ...
        for split in splits:
            for key in put_data.keys():
                ...
                for i in range(len(self.dataframe[split])):
                    if self.tags[key] in [
                        ...
                        array = getattr(array_creator, f"create_{self.tags[key]}")(
                            ...
                            self.dataframe[split].loc[
                                i, f"{key}_{self.tags[key]}_slice"
                            ],
                ...
                if self.tags[key] == "object_detection":
                    for j in range(num_arrays):
                        out_array[split][key + j] = np.array(
                            globals()[f"current_arrays_{j + 1}"]
                        )
                else:
    ...
    def create_dataset_configure(self, creation_data: CreationData) -> dict:
        ...
        for path, dirs, files in os.walk(
            os.path.join(self.trds_path, f"dataset {self.name}")
        ):
            for file in files:
        ...
        with open(
            os.path.join(self.trds_path, f"dataset {self.name}", "config.json"), "w"
        ) as fp:
            json.dump(data, fp)

        return data

    def instructions_image(
        self,
        paths_list: list,
        put_data: Union[CreationInputData, CreationOutputData, OneFileData],
    ):
        ...
        for key, value in self.tags.items():
            if value == "classification":
                if self.user_parameters[key].sources_paths and os.path.isfile(
                    self.user_parameters[key].sources_paths[0]
                ):
                    data = pd.read_csv(
                        self.user_parameters[key].sources_paths[0],
        ...
        if options.get("augmentation", None):
            ...
            for key, value in put_data.parameters.augmentation.__dict__.items():
                try:
                    ...
                    pass
            array_creator.augmentation[put_data.id] = iaa.Sequential(
                aug_parameters, random_order=True
            )
    ...
    def instructions_video(
        self, paths_list: list, put_data: Union[CreationInputData, CreationOutputData]
    ):
        """
        Args:
            paths_list: list
            ...
            if options["video_mode"] == VideoModeChoice.completely:
                ...
                while not stop_flag:
                    ...
                    if cur_class != prev_class:
                        self.peg.append(peg_idx)
                        prev_class = cur_class
                    y_cls.append(csv_y_cls[idx]) if csv_flag else y_cls.append(
                        cur_class
                    )
                    ...
                    if cur_step + options["length"] > frame_count:
                        ...
                        if options["length"] < frame_count:
                            video.append(elem)
                            video_slice.append(
                                [frame_count - options["length"], frame_count]
                            )
                            y_cls.append(csv_y_cls[idx]) if csv_flag else y_cls.append(
                                cur_class
                            )
    ...
    def instructions_text(
        ...
        def read_text(
            file_path, lower, del_symbols, split, open_symbol=None, close_symbol=None
        ) -> str:

            with open(
                os.path.join(self.file_folder, file_path), "r", encoding="utf-8"
            ) as txt:
                text = txt.read()
            ...
            text = " ".join(
                text_to_word_sequence(
                    text, **{"lower": lower, "filters": del_symbols, "split": split}
                )
            )
        ...
        for idx, path in enumerate(paths_list):
            if not csv_flag:
                ...
                txt_list[path] = read_text(
                    path, lower, filters, split, open_symbol, close_symbol
                )
            else:
        ...
        if options.get("word_to_vec", bool):
            ...
            array_creator.create_word2vec(
                put_data.id,
                txt_list_w2v,
                **{
                    "size": options.get("word_to_vec_size", ""),
                    "window": 10,
                    "min_count": 1,
                    "workers": 10,
                    "iter": 10,
            ...
            array_creator.create_tokenizer(
                put_data.id,
                **{
                    "num_words": options.get("max_words_count", int),
                    "filters": filters,
                    "lower": lower,
                    "split": split,
                    "char_level": False,
        ...
        for idx, (key, value) in enumerate(sorted(txt_list.items())):
            ...
            if options["text_mode"] == TextModeChoice.completely:
                ...
                while not stop_flag:
                    text.append(
                        " ".join(
                            value.split(" ")[cur_step : cur_step + options["length"]]
                        )
                    )
        ...
        instructions["parameters"] = {
            "embedding": options.get("embedding", bool),
    ...
    def instructions_audio(
        ...
        for idx, elem in enumerate(paths_list):
            ...
            if options["audio_mode"] == AudioModeChoice.completely:
                ...
                stop_flag = False
                y, sr = librosa_load(
                    path=os.path.join(self.file_folder, elem),
                    sr=sample_rate,
                    res_type="scipy",
                )
        ...
        if "classification" in self.tags.values():
            ...
            if options["pad_sequences"] or options["xlen_step"]:
                ...
                for i in range(len(df_with_y.loc[:, y_col[0]]) - 1):
                    if (
                        df_with_y.loc[:, y_col[0]][i]
                        != df_with_y.loc[:, y_col[0]][i + 1]
                    ):
                        self.peg.append(i + 1)
                ...
                for i in range(
                    ...
                    if (
                        abs(
                            (
                                y_subdf.iloc[
                                    i + instructions["parameters"]["length"] + 1
                                ][0]
                                - y_subdf.iloc[i][0]
                            )
                            / y_subdf.iloc[i][0]
    ...
    def instructions_segmentation(
        ...
        for key, value in self.tags.items():
            if value == "image":
                shape = (
                    self.user_parameters.get(key).height,
                    self.user_parameters.get(key).width,
                )
    ...
    def instructions_object_detection(
        self, paths_list: list, put_data: Union[CreationInputData, CreationOutputData]
    ):
    ...
    def instructions_text_segmentation(
        self, _, put_data: Union[CreationInputData, CreationOutputData]
    ):
        ...
        for i, value in self.tags.items():
            if value == "text":
                for key, txt_file in self.temporary[i].items():
                    if txt_file:
                        text_instr, segment_instr = get_samples(
                            txt_file, open_tags, close_tags
                        )
                ...
                for idx in range(len(text)):
                    if text_mode == TextModeChoice.completely:
                        ...
                        while not stop_flag:
                            text_sliced.append(
                                " ".join(text[idx][cur_step : cur_step + length])
                            )
                            text_segm_data.append(
                                text_segm[idx][cur_step : cur_step + length]
                            )
                ...
                self.build_dataframe[
                    f"{put_data.id}_text_segmentation"
                ] = text_segm_data
                self.build_dataframe[
                    f"{put_data.id}_text_segmentation_slice"
                ] = text_segm_sliced

            break

        self.peg.append(len(text_segm_sliced))
        instructions["parameters"] = {"num_classes": len(open_tags), "put": put_data.id}
        if options.get("deploy", bool):
            instructions["instructions"] = {
                f"{put_data.id}_text_segmentation": text_segm_data,
