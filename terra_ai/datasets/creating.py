...
class CreateDTS(object):
    ...
    def create_instructions(
        ...
        data: Union[CreationInputsList, CreationOutputsList],
    ):
        self.iter = 0
        ...
        for elem in data:
            self.iter += 1
            self.instructions[instruction_key][elem.alias] = getattr(
                self, f"instructions_{elem.type}"
    ...
    def create_dataset(self, creation_data: CreationData):
        ...
        for key in self.instructions["outputs"].keys():
            ...
            if isinstance(array, tuple):
                for i in range(len(array)):
                    self.output_shape[
                        key.replace(key[-1], str(int(key[-1]) + i))
                    ] = array[i].shape
                    self.output_dtype[
                        key.replace(key[-1], str(int(key[-1]) + i))
                    ] = str(array[i].dtype)
        ...
        if dataset_dict["parameters"]["use_generator"]:
            ...
            os.makedirs(
                os.path.join(self.trds_path, f"dataset {self.name}", "instructions"),
                exist_ok=True,
            )
            for key in self.instructions.keys():
                os.makedirs(
                    os.path.join(
                        self.trds_path, f"dataset {self.name}", "instructions", key
                    ),
                    exist_ok=True,
                )
            ...
            for key in self.instructions["inputs"].keys():
                ...
                for i in range(self.limit):
                    x.append(
                        getattr(array_creator, f"create_{self.tags[key]}")(
                ...
                if "object_detection" in self.tags.values():
                    ...
                    for spl_seq in splits:
                        for i in range(len(splits)):
                            self.Y[spl_seq][
                                key.replace(key[-1], str(int(key[-1])))
                            ] = np.array(y_1)[self.split_sequence[spl_seq]]
                            self.Y[spl_seq][
                                key.replace(key[-1], str(int(key[-1]) + 1))
                            ] = np.array(y_2)[self.split_sequence[spl_seq]]
                            self.Y[spl_seq][
                                key.replace(key[-1], str(int(key[-1]) + 2))
                            ] = np.array(y_3)[self.split_sequence[spl_seq]]
                else:
                    y: list = []
                    for i in range(self.limit):
                        y.append(
            ...
            for sample in self.X.keys():
                os.makedirs(
                    os.path.join(self.trds_path, "arrays", sample), exist_ok=True
                )
                for inp in self.X[sample].keys():
                    os.makedirs(
                        os.path.join(
                            self.trds_path, f"dataset {self.name}", "arrays", sample
                        ),
                        exist_ok=True,
                    )
                    joblib.dump(
        ...
        if array_creator.scaler:
            os.makedirs(
                os.path.join(self.trds_path, f"dataset {self.name}", "scalers"),
                exist_ok=True,
            )
        if array_creator.tokenizer:
            os.makedirs(
                os.path.join(self.trds_path, f"dataset {self.name}", "tokenizer"),
                exist_ok=True,
            )
        if array_creator.word2vec:
            os.makedirs(
                os.path.join(self.trds_path, f"dataset {self.name}", "word2vec"),
                exist_ok=True,
            )
        ...
        for tok in array_creator.tokenizer.keys():
            if array_creator.tokenizer[tok]:
                joblib.dump(
                    array_creator.tokenizer[tok],
                    os.path.join(
                        self.trds_path, f"dataset {self.name}", "tokenizer", f"{tok}.gz"
                    ),
                )
        for w2v in array_creator.word2vec.keys():
            if array_creator.word2vec[w2v]:
                joblib.dump(
                    array_creator.word2vec[w2v],
                    os.path.join(
                        self.trds_path, f"dataset {self.name}", "word2vec", f"{w2v}.gz"
                    ),
                )
    ...
    def instructions_images(self, **options):
        ...
        if "object_detection" in self.tags.values():
            for file_name in sorted(
                os.listdir(os.path.join(self.file_folder, options["folder_name"]))
            ):
            ...
            if file_name:
                ...
                for name in sorted(file_name):
                    ...
                    if options["class_mode"] == "По каждому кадру":
                        y_cls.append(
                            np.full((options["max_frames"], 1), cls_idx).tolist()
                        )
                    else:
                        y_cls.append(cls_idx)
                cls_idx += 1
    ...
    def instructions_text(self, **options):
        def read_text(file_path):
            ...
            for put, tag in self.tags.items():
                if tag == "text_segmentation":
                    open_symbol = self.user_parameters[put]["open_tags"].split(" ")[0][
                        0
                    ]
                    close_symbol = self.user_parameters[put]["open_tags"].split(" ")[0][
                        -1
                    ]
        ...
        array_creator.create_tokenizer(
            ...
            self.iter,
            **{
                "num_words": options["max_words_count"],
                "filters": filters,
                "lower": True,
                "split": " ",
                "char_level": False,
        ...
        array_creator.tokenizer[f"{self.mode}_{self.iter}"].fit_on_texts(
            list(txt_list.values())
        )
        ...
        for key, value in txt_list.items():
            array_creator.txt_list[f"{self.mode}_{self.iter}"][
                key
            ] = array_creator.tokenizer[f"{self.mode}_{self.iter}"].texts_to_sequences(
                [value]
            )[
                0
            ]
        ...
        if options["word_to_vec"]:
            reverse_tok = {}
            for key, value in array_creator.tokenizer[
                f"{self.mode}_{self.iter}"
            ].word_index.items():
        ...
        if "text_segmentation" not in self.tags.values():
            ...
            self.peg.append(0)
            for key in sorted(
                array_creator.txt_list[f"{self.mode}_{self.iter}"].keys()
            ):
                index = 0
    ...
    def instructions_classification(self, **options):
        ...
        for key, value in self.tags.items():
            if value in ["image", "text", "audio", "video"]:
                self.classes_names[f"{self.mode}_{self.iter}"] = (
                    sorted(os.listdir(self.file_folder))
                    if not self.user_parameters[key]["folder_name"]
                    else list(self.user_parameters[key]["folder_name"].split())
                )
                self.num_classes[f"{self.mode}_{self.iter}"] = len(
                    self.classes_names[f"{self.mode}_{self.iter}"]
                )
            elif value in ["dataframe"]:
                self.classes_names[f"{self.mode}_{self.iter}"] = self.temporary[
                    "classes_names"
                ]
                self.num_classes[f"{self.mode}_{self.iter}"] = self.temporary[
                    "num_classes"
                ]

        instructions: dict = {
            "parameters": {
                "num_classes": len(np.unique(self.y_cls)),
    ...
    def instructions_segmentation(self, **options):
        ...
        instructions = {
            ...
            "parameters": {
                ...
                "shape": (
                    self.user_parameters["input_1"]["height"],
        ...
        for key, value in self.tags.items():
            if value == "text":
                tags_indexes = np.array(
                    [array_creator.tokenizer[key].word_index[idx] for idx in tags]
                )
                for txt_file in array_creator.txt_list[key].keys():
                    text_instr, segment_instr = get_ohe_samples(
                        array_creator.txt_list[key][txt_file], tags_indexes
                    )
                    array_creator.txt_list[f"{self.mode}_{self.iter}"][
                        txt_file
                    ] = segment_instr
