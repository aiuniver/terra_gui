...
class DataList(DataBaseList):
    ...
    def preset_update(self, data):
        predict_path = str(Path(self.path_deploy, data.get("predict")))
        with open(predict_path) as predict_ref:
            data.update(
                {
                    "source": str(Path(self.path_deploy, data.get("source"))),
                    "predict": predict_ref.read(),
                }
            )
        return data
    ...
    def update(self, index: int):
        ...
        destination_source = Path(self.source_path, f"{index + 1}.webm")
        destination_predict = Path(self.predict_path, f"{index + 1}.txt")
